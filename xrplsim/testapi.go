...
import (
	...
	"github.com/xrpl-commons/xrpl-hive/internal/simapi"
)
...
// AnyTest is a TestSpec, ClientTestSpec, or NetworkTestSpec.
...
//
//	c := t.StartClient(clientType)
//	// use c.RPCEndpoint() or c.WSEndpoint() to connect
...
// NetworkTestSpec runs a test with multiple clients in a shared network.
// This is the primary test type for XRPL interoperability testing, where
// multiple validator nodes must form a consensus network.
type NetworkTestSpec struct {
	...
	// Role filters client types.
	Role string

	// The Run function receives a T and all started clients.
	Run func(*T, []*Client)
}
...
// RPCEndpoint returns the HTTP JSON-RPC endpoint of the XRPL client.
func (c *Client) RPCEndpoint() string {
	return fmt.Sprintf("http://%v:5005", c.IP)
}

// WSEndpoint returns the WebSocket endpoint of the XRPL client.
func (c *Client) WSEndpoint() string {
	return fmt.Sprintf("ws://%v:6006", c.IP)
}
...
func (spec ClientTestSpec) runTest(host *Simulation, suiteID SuiteID, suite *Suite) error {
	...
	for _, clientDef := range clients {
		// 'role' is an optional filter, so different client roles
...
func (spec NetworkTestSpec) runTest(host *Simulation, suiteID SuiteID, suite *Suite) error {
	clients, err := host.ClientTypes()
	if err != nil {
		return err
	}
	...
	var matchingClients []*ClientDefinition
	for _, clientDef := range clients {
		if spec.Role != "" && !clientDef.HasRole(spec.Role) {
			continue
		}
		matchingClients = append(matchingClients, clientDef)
	}
	...
	return runTest(host, test, func(t *T) {
		// Start one instance of each matching client type.
		var started []*Client
		for _, clientDef := range matchingClients {
			c := t.StartClient(clientDef.Name)
