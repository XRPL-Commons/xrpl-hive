...
func (api *simAPI) startClient(w http.ResponseWriter, r *http.Request) {
	...
	if env["XRPL_LOGLEVEL"] == "" {
		env["XRPL_LOGLEVEL"] = strconv.Itoa(api.env.SimLogLevel)
	}
	...
	// by default: check the XRPL RPC port
	options.CheckLive = 5005
	if portStr := env["XRPL_CHECK_LIVE_PORT"]; portStr != "" {
