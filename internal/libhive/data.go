...
const (
	LabelHiveInstance    = "xrplhive.instance"     // Unique Hive instance ID
	LabelHiveVersion     = "xrplhive.version"      // Hive version/commit
	LabelHiveType        = "xrplhive.type"         // container type: client|simulator|proxy
	LabelHiveTestSuite   = "xrplhive.test.suite"   // test suite ID
	LabelHiveTestCase    = "xrplhive.test.case"    // test case ID
	LabelHiveClientName  = "xrplhive.client.name"  // client name (rippled, goxrpl, rxrpl, etc)
	LabelHiveClientImage = "xrplhive.client.image" // Docker image name
	LabelHiveCreated     = "xrplhive.created"      // RFC3339 timestamp
	LabelHiveSimulator   = "xrplhive.simulator"    // simulator name
)
...
func GenerateHiveInstanceID() string {
	return fmt.Sprintf("xrplhive-%d-%s", os.Getpid(), time.Now().Format("20060102-150405.000"))
}
...
func GenerateContainerName(containerType, identifier string) string {
	...
	if identifier != "" {
		...
		return fmt.Sprintf("xrplhive-%s-%s-%d-%s", sanitizedType, sanitizedIdentifier, counter, timestamp)
	}
	return fmt.Sprintf("xrplhive-%s-%d-%s", sanitizedType, counter, timestamp)
}
