...
import (
	...
	"github.com/xrpl-commons/xrpl-hive/internal/libdocker"
	"github.com/xrpl-commons/xrpl-hive/internal/libhive"
...
func main() {
	var (
		...
		dockerAuth      = flag.Bool("docker.auth", false, "Enable docker authentication from system config files.")
		...
		simTimeLimit    = flag.Duration("sim.timelimit", 0, "Simulation `timeout`. xrpl-hive aborts the simulator if it exceeds this time.")
		...
		// Cleanup flags
		cleanupContainers = flag.Bool("cleanup", false, "Clean up xrpl-hive containers instead of running simulations")
		cleanupDryRun     = flag.Bool("cleanup.dry-run", false, "Show what containers would be cleaned up without actually removing them")
		cleanupInstance   = flag.String("cleanup.instance", "", "Clean up containers from specific xrpl-hive instance ID only")
		...
		listContainers    = flag.Bool("list", false, "List xrpl-hive containers instead of running simulations")

		clientsFile = flag.String("client-file", "", "YAML `file` containing client configurations.")

		clients = flag.String("client", "rippled", "Comma separated `list` of clients to use. Client names in the list may be given as\n"+
			...
			"Example: \"rippled_latest,goxrpl_main,rxrpl_latest\"\n")
		...
		clientTimeout = flag.Duration("client.checktimelimit", 3*time.Minute, "The `timeout` of waiting for clients to open up the RPC port.\n"+
			...
			"A lower value means that xrpl-hive won't wait as long in case the node crashes and\n"+
